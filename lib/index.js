 ...
 *   blockScoping - Compile ES6 Block Scopes (const, let).
...
function transform(ast, options) {
  ...
  // Must be first in pipeline so it
  // has the (correct) node.range
  if (options.blockScoping !== false) {
    // defs expects ast.program not ast
    ast = es6blocks(ast.program, {
      ast: true,
    ...
    // reconstruct esnext standard ast before
    // passing it on
    ast = {
      program: ast.ast,
      type: 'File'
